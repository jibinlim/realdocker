		...
		public class Service
		{
			...
			public async Task<Boolean> Connect()
			{
				try
				{
					...
					//mqttClient.Subscribe(new string[] { "aseado/mes/vision" },
                ...
                while (true)
				{
					try
					{
						...
						foreach (var address in WordAddressList)
						{
							...
							{
								// todo DWord 로 하면 에러남...
								//val = xGTClass.Read(XGT_DataType.Word, address.Key, XGT_MemoryType.KeepRelay_K, 0);
								//if (val.DataList != null)
								//{
								//	ReceiveData newdata = new ReceiveData
								//	{
